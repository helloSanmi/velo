...
type Row =
  ...
  | { key: string; source: 'directory'; provider: 'microsoft'; displayName: string; email: string; licensed: false; entry: { email: string; displayName: string; firstName?: string; lastName?: string } };

interface SettingsAdminUsersSectionProps {
  org?: { microsoftWorkspaceConnected?: boolean } | null;
  ...
  onSyncDirectory: (provider: 'microsoft') => void;
            ...
            <tbody>
              {props.filteredRows.map((row) => (
                <tr key={row.key} className="border-t border-slate-200">
                  ...
                  <td className="px-2 py-1.5"><span className="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] text-slate-700">{row.source === 'workspace' ? 'Workspace' : 'Microsoft directory'}</span></td>
